...
public:
	...
	void updateEmptyCells(int x, int y, bool remove);
...
void TicTacToe::removeMove(int row, int col)
{
	board[row][col] = ' ';
	noOfMoves--;
	updateEmptyCells(row, col, false);
}

void TicTacToe::updateEmptyCells(int x, int y, bool remove)
{
	if (remove)
		emptyCells.remove(x * 3 + y);
	else
		emptyCells.push_back(x * 3 + y);
}
/*********************************
